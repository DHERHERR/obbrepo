OBB ECHO
# Open the Python environment with:
# > py
...
STATIONS_DATA = [
    {"estacion": "Hauptbahnhof", "ciudad": "Viena", "lat": 48.186667, "lon": 16.38, "linea": "Main East Node"},
    {"estacion": "Mitte (Landstrasse)", "ciudad": "Viena", "lat": 48.206389, "lon": 16.384722, "linea": "S-Bahn / CAT"},
    {"estacion": "Floridsdorf", "ciudad": "Viena", "lat": 48.256389, "lon": 16.4, "linea": "North / S-Bahn"},
    ...
    {"estacion": "Traisengasse", "ciudad": "Viena", "lat": 48.2358, "lon": 16.3826, "linea": "Main Track"},
    {"estacion": "Kaiserebersdorf", "ciudad": "Viena", "lat": 48.146039, "lon": 16.465389, "linea": "S7"},
    {"estacion": "Matzleinsdorfer Platz", "ciudad": "Viena", "lat": 48.1801806, "lon": 16.3581194, "linea": "Main Track"},
    {"estacion": "Ottakring", "ciudad": "Viena", "lat": 48.2117111, "lon": 16.3112111, "linea": "Suburban Line"},
    ...
    {"estacion": "Gersthof", "ciudad": "Viena", "lat": 48.23125, "lon": 16.329, "linea": "Suburban Line"},
    {"estacion": "Nussdorf", "ciudad": "Viena", "lat": 48.25995, "lon": 16.36793, "linea": "Franz-Josefs Railway"},
    {"estacion": "Spittelau", "ciudad": "Viena", "lat": 48.2354, "lon": 16.3581889, "linea": "S40 / S45"},
    {"estacion": "Simmering", "ciudad": "Viena", "lat": 48.1701389, "lon": 16.41975, "linea": "Laa Railway"},
...
def score_to_emoji_color(score):
    # pd.NA does not support boolean comparisons; normalize with pd.isna
...
def nube_palabras(opiniones: pd.DataFrame, estacion: str, top_n: int = 15):
    """Returns top_n words by frequency for a station."""
    if opiniones.empty:
        return []
    ...
    stop = {
        ...
        "con", "del", "que", "para", "muy", "esto", "eso", "esta", "ese",
        "was", "were", "is", "are", "very", "good", "bad", "ok"
    }
    counter = Counter()
    for t in textos:
        words = re.findall(r"[A-Za-záéíóúüñ']{3,}", t)
...
def render_word_cloud(words_with_freq):
    """Renders a simple word cloud using HTML with size proportional to frequency."""
...
def vista_mapa():
    ...
    st.write(
        "Visualize the satisfaction per station. Each emoji on the map reflects the average sentiment."
    )
    ...
    st.subheader("💬 Word cloud (top 15 terms)")
...
def vista_encuesta():
    st.title("📝 Station Experience Survey")
    ...
    st.markdown("---")
    st.subheader("🗂️ Recent opinions")
    opiniones_df = cargar_opiniones()
